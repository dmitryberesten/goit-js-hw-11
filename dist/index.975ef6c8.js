...
function onSearch(evt) {
    ...
    if (name !== "") pixabay(name); // отримати зображення
    else {
...
// отримання зображень
async function pixabay(name, page1) {
    ...
    try {
        ...
        const response = await (0, _axiosDefault.default).get(API_URL, options);
        // сповіщення notiflix
        notification(response.data.hits.length, response.data.total // отримання кількості
        );
...
function createMarkup(arr) {
    const markup = arr.hits.map((item)=>`<a class="photo-link" href="${item.largeImageURL}">
        ...
        </a>`).join(""); // сполучення рядків всіх об'єктів (всіх картинок)
...
},{"axios":"jo6P5","notiflix":"5z0Oc","simplelightbox":"9ydBq","simplelightbox/dist/simple-lightbox.min.css":"kaxSc","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"jo6P5":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
